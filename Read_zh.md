...
- **创建新文件**：支持基础文本内容，通过对话框输入文件名和内容。
- **文件查看**：双击文件或右键选择"查看"，以只读模式安全查看文件内容，避免意外修改。
- **文件编辑**：右键选择"编辑"，在专用编辑对话框中修改文件内容并保存。
- **删除文件**：自动校验只读属性，只读文件禁止删除。
- **文件重命名**：右键选择"重命名"，修改文件名称。
- **文件属性查看**：右键选择"属性"，查看文件详细信息（大小、创建时间、磁盘位置等）。

#### 新增功能亮点
- **查看与编辑分离**：双击文件默认为安全的只读查看模式，需要编辑时通过右键菜单选择"编辑"。
- **用户体验优化**：避免误操作修改文件，同时保留完整的编辑功能。
- **多种操作方式**：支持双击、右键菜单、菜单栏等多种操作方式。
...
- **多视图模式**：支持列表视图和图标视图两种显示模式，满足不同使用习惯。
- **智能交互设计**：
  - 双击文件：默认以只读模式查看文件内容
  - 双击目录：进入目录
  - 右键菜单：提供查看、编辑、删除、属性等完整操作选项
  - 菜单栏：文件操作、视图切换、工具选项
- **对话框系统**：
  - 文件查看对话框：只读模式，安全查看文件内容
  - 文件编辑对话框：完整编辑功能，支持保存和取消
  - 新建文件对话框：创建新文件
  - 重命名对话框：文件/目录重命名
  - 属性对话框：查看详细信息
...
### 详细文档
- **[技术架构文档](./doc/architecture.md)**：系统设计和架构详解
- **[API参考文档](./doc/api-reference.md)**：核心类和方法的详细说明
- **[用户操作手册](./doc/user-manual.md)**：完整的用户使用指南
- **[开发指南](./doc/development-guide.md)**：开发环境搭建和扩展指南

### 快速链接
- 🚀 [快速开始](./doc/user-manual.md#快速开始)
- 📖 [用户界面介绍](./doc/user-manual.md#界面介绍)
- 🔧 [开发环境搭建](./doc/development-guide.md#开发环境搭建)
- 📚 [API参考](./doc/api-reference.md#核心api接口)
